/**
 * Template Validation Rules for WhatsApp Message Templates (Frontend)
 * Mirror of backend validation for real-time client-side validation
 *
...
export interface ValidationIssue {
  rule: ValidationRule
  messageId: string // Key for i18n translation
 ...
 */
function checkHeaderTextTooLong(
  text: string,
  maxLength: number = 60
): ValidationIssue | null {
  if (text.length > maxLength) {
 ...
 */
function checkAuthNoMediaHeader(
 ...
 */
function checkAuthRequiresOtpButton(
  buttons: Array<{ type: string; otp_type?: string }> | null | undefined
): ValidationIssue | null {
  ...
  const hasOtpButton = buttons.some(
    (btn) =>
      btn.type === "OTP" ||
      btn.type === "COPY_CODE" ||
...
export function validateTemplateContent(
  options: ValidateTemplateContentOptions
): ValidationResult {
  const { content, category, footerContent, headerContent, headerType, buttons } =
    options
...
/**
 * Validate only the body content (for real-time validation while typing)
 */
...
/**
 * Get all positions that should be highlighted in preview
 */
export function getHighlightPositions(
  ...
  for (const issue of issues) {
    positions.push(...issue.positions)
  }

  // Sort by start position and merge overlapping ranges
  positions.sort((a, b) => a.start - b.start)

  const merged: ValidationPosition[] = []
...
/**
 * Render content with highlighted error positions as React nodes
 */
export function renderContentWithHighlights(
  content: string,
  issues: ValidationIssue[]
): Array<{ text: string; highlighted: boolean; issueRule?: ValidationRule }> {
  const positions = getHighlightPositions(content, issues)
  ...
  for (const pos of positions) {
    ...
    if (pos.start > lastEnd) {
      segments.push({
        text: content.slice(lastEnd, pos.start),
        highlighted: false,
      })
    }

    // Find the rule for this position
    const matchingIssue = issues.find((issue) =>
      issue.positions.some(
        (p) =>
          (pos.start >= p.start && pos.start < p.end) ||
          (p.start >= pos.start && p.start < pos.end)
      )
    )
    ...
    segments.push({
      ...
      highlighted: true,
      issueRule: matchingIssue?.rule,
    })

    lastEnd = pos.end
  }

  // Add remaining text after last position
